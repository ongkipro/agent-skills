...
Always import from `@shopify/ui-extensions/checkout` — **never** from `@shopify/checkout-ui-extensions`, `@shopify/checkout-ui-extensions-react`, `@shopify/polaris-web-components`, or any other package. Those packages do not exist.

```ts
import { extension, Banner, BlockStack } from '@shopify/ui-extensions/checkout';
```

For React/Preact-based extensions:

```tsx
import { reactExtension, Banner, BlockStack } from '@shopify/ui-extensions/checkout';
import { useExtensionCapability } from '@shopify/ui-extensions-react/checkout';
```
...
If validation fails with an unknown prop or type error, search for the correct values before retrying:
```
