...
Always import from `@shopify/ui-extensions/admin` — **never** from `@shopify/admin-ui-extensions-react`, `@shopify/admin-ui-extensions`, `@shopify/polaris-web-components`, or any other package. Those packages do not exist.

```ts
import { extension, AdminAction } from '@shopify/ui-extensions/admin';
```

For Preact-based entry points:
